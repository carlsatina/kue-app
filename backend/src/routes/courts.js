...
router.get("/", requireAuth, requireRole(["admin", "staff"]), async (req, res) => {
  const courts = await prisma.court.findMany({ where: { deletedAt: null } });
  res.json(courts);
});
...
router.post("/", requireAuth, requireRole(["admin"]), async (req, res) => {
  ...
  const court = await prisma.court.create({ data: parse.data });

  const activeSession = await prisma.session.findFirst({ where: { status: "open" } });
  if (activeSession) {
...
router.post("/:id/status", requireAuth, requireRole(["admin", "staff"]), async (req, res) => {
  ...
  const courtSession = await prisma.courtSession.update({
    where: { sessionId_courtId: { sessionId, courtId: req.params.id } },
    data: { status }
  });
